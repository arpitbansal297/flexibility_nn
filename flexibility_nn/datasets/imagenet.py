...
from flexibility_nn.datasets.utils import check_integrity, extract_archive, verify_str_arg
...
class ImageNet(ImageFolder):
    ...
    def __init__(self, root: str, root1: str, split: str = "train", **kwargs: Any) -> None:
        root = self.root = os.path.expanduser(root)
        root1 = self.root1 = os.path.expanduser(root1)
        self.split = verify_str_arg(split, "split", ("train", "val"))

        self.parse_archives()
        wnid_to_classes = load_meta_file(self.root1)[0]
    ...
    def parse_archives(self) -> None:
        if not check_integrity(os.path.join(self.root1, META_FILE)):
            parse_devkit_archive(self.root1)

        if not os.path.isdir(self.split_folder):
            if self.split == "train":
                parse_train_archive(self.root1)
            elif self.split == "val":
                parse_val_archive(self.root1)
