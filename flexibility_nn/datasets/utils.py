...
def save_histogram(dataloader, bins=50, filename='./pixel_histogram2.png'):

    """
    Compute and save the histogram of pixel values from images in a DataLoader.

    Parameters:
    - dataloader (torch.utils.data.DataLoader): The DataLoader instance containing the images.
    - bins (int): Number of bins for the histogram.
    - filename (str): The name of the file where the histogram will be saved.
    """
    ...
    # Loop through batches of images and compute the histogram
    for batch, y in dataloader:
        # Reshape the batch to a 1D array
        batch_1d = batch.cpu().numpy().ravel()

        # Append the data of this batch to the overall data
        hist_data = np.concatenate([hist_data, batch_1d])

    # Plot and save the histogram
    plt.hist(hist_data, bins=bins, density=True, alpha=0.75)
